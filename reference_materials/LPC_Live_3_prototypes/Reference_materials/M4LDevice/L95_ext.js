...
function update(args){
    ...
    if (info_0) { info_0.message("set", info[0]); }
    if (info_1) { info_1.message("set", info[1]); }

    if (att_0) { att_0.message("set", attributes[0]); }
    if (att_1) { att_1.message("set", attributes[1]); }
    if (att_2) { att_2.message("set", attributes[2]); }
    if (att_3) { att_3.message("set", attributes[3]); }
    if (att_4) { att_4.message("set", attributes[4]); }
    if (att_5) { att_5.message("set", attributes[5]); }
    if (att_6) { att_6.message("set", attributes[6]); }
    if (att_7) { att_7.message("set", attributes[7]); }

    if (att_n_0) { att_n_0.message("set", attribute_names[0]); }
    if (att_n_1) { att_n_1.message("set", attribute_names[1]); }
    if (att_n_2) { att_n_2.message("set", attribute_names[2]); }
    if (att_n_3) { att_n_3.message("set", attribute_names[3]); }
    if (att_n_4) { att_n_4.message("set", attribute_names[4]); }
    if (att_n_5) { att_n_5.message("set", attribute_names[5]); }
    if (att_n_6) { att_n_6.message("set", attribute_names[6]); }
    if (att_n_7) { att_n_7.message("set", attribute_names[7]); }
...
function compute_screenshots_dir(){
    ...
    if (idx >= 0) {
        ...
        screenshots_dir = base + "/../Launchpad95 Mode Screenshots/";
    }
}
...
function update_mode_assets(mode_id_val){
    ...
    if (mode_id_val != last_mode_id) { last_mode_id = mode_id_val; }
...
function set_box_rect(box, rect){
    ...
    try { box.message("presentation_rect", rect[0], rect[1], rect[2], rect[3]); } catch (e3) { }
    try { box.message("patching_rect", rect[0], rect[1], rect[2], rect[3]); } catch (e4) { }
}
...
function set_bp_rect(bp, x, y, w, h){
    if (bp) {
        ...
        try { bp.message("presentation_rect", x, y, w, h); } catch (e3) { }
        try { bp.message("patching_rect", x, y, w, h); } catch (e4) { }
    }
}
...
function apply_layout(data){
    ...
    capture_info_base();
    var base_w = sane_dim(info_content_w, 420, 200, 2000);
    var base_h = sane_dim(info_content_h, 60, 40, 400);

    var info_mx = Math.max(10, Math.round(base_w * margin_ratio));
    var info_my = Math.max(10, Math.round(base_h * margin_ratio));
    var info_w = base_w + (info_mx * 2);
    var info_h = base_h + (info_my * 2) + 2;

    var guide_img_w = sane_dim(data.w, 720, 200, 2000);
    var guide_img_h = sane_dim(data.h, 620, 200, 2000);

    var guide_mx = Math.max(10, Math.round(guide_img_w * margin_ratio));
    var guide_my = Math.max(10, Math.round(guide_img_h * margin_ratio));
    var guide_w = guide_img_w + (guide_mx * 2);
    var guide_h = guide_my + guide_label_height + guide_my + guide_img_h + guide_my;

    var mi_content_w = Math.max(guide_img_w, base_w, 420);
    var mi_mx = Math.max(10, Math.round(mi_content_w * margin_ratio));
    var mi_my = Math.max(10, Math.round(info_box_height * margin_ratio));
    var mi_w = mi_content_w + (mi_mx * 2);
    var mi_h = mi_my + info_label_height + mi_my + info_box_height + mi_my;
    ...
    if (show_info_section) {
        content_w = Math.max(content_w, info_w);
        content_h += info_h;
        visible += 1;
    }
    if (show_guide_section) {
        content_w = Math.max(content_w, guide_w);
        content_h += guide_h;
        visible += 1;
    }
    if (show_modeinfo_section) {
        content_w = Math.max(content_w, mi_w);
        content_h += mi_h;
        visible += 1;
    }

    if (visible > 1) {
        content_h += section_gap * (visible - 1);
    }

    content_w = sane_dim(content_w, 600, 320, 2400);
    content_h = sane_dim(content_h, 400, 160, 2400);

    var pad_w = Math.max(10, Math.round(content_w * margin_ratio));
    var pad_h = Math.max(10, Math.round(content_h * margin_ratio));

    var x = pad_w;
    var y = menu_height + pad_h;

    if (show_info_section && section_info_bp) {
        apply_info_margin(info_mx, info_my);
        set_bp_rect(section_info_bp, x, y, info_w, info_h);
        y += info_h + section_gap;
    }

    if (show_guide_section && section_guide_bp) {
        set_bp_rect(section_guide_bp, x, y, guide_w, guide_h);
        if (mode_guide_label) {
            set_box_rect(mode_guide_label, [guide_mx, guide_my, 220, guide_label_height]);
        }
        if (mode_guide_pic) {
            set_box_rect(mode_guide_pic, [guide_mx, guide_my + guide_label_height + guide_my, guide_img_w, guide_img_h]);
        }
        y += guide_h + section_gap;
    }

    if (show_modeinfo_section && section_modeinfo_bp) {
        set_bp_rect(section_modeinfo_bp, x, y, mi_w, mi_h);
        if (mode_info_label) {
            set_box_rect(mode_info_label, [mi_mx, mi_my, 220, info_label_height]);
        }
        if (mode_info_box) {
            set_box_rect(mode_info_box, [mi_mx, mi_my + info_label_height + mi_my, mi_content_w, info_box_height]);
        }
        y += mi_h + section_gap;
    }

    var win_w = content_w + (pad_w * 2);
    var win_h = menu_height + content_h + (pad_h * 2);

    win_w = sane_dim(win_w, 600, 360, 2600);
    win_h = sane_dim(win_h, 400, menu_height + 160, 2600);
...
function show_info(v){
    show_info_section = (v != 0);
    apply_section_visibility();
    apply_layout(MODE_MAP[last_mode_id] || MODE_MAP["session"]);
}

function show_guide(v){
    show_guide_section = (v != 0);
    apply_section_visibility();
    apply_layout(MODE_MAP[last_mode_id] || MODE_MAP["session"]);
}

function show_modeinfo(v){
    show_modeinfo_section = (v != 0);
    apply_section_visibility();
    apply_layout(MODE_MAP[last_mode_id] || MODE_MAP["session"]);
}
...
function apply_layout(data){
    ...
    var guide_h = section_pad + guide_label_height + section_pad + guide_img_h + section_pad;
    ...
    var mi_h = mi_pad + info_label_height + mi_pad + info_box_h + mi_pad;
    var mi_w = section_w;
    ...
    for (var s = 0; s < sections.length; s++) {
        ...
        if (type == "info") {
            ...
            if (mode_guide_pic) {
                set_box_rect(mode_guide_pic, [section_pad, section_pad + guide_label_height + section_pad, guide_img_w, guide_img_h]);
            }
            ...
            if (mode_info_label) {
                set_box_rect(mode_info_label, [mi_pad, mi_pad, 220, info_label_height]);
            }
            if (mode_info_box) {
                set_box_rect(mode_info_box, [mi_pad, mi_pad + info_label_height + mi_pad, mi_content_w, info_box_h]);
            }
            send_child_rect(section_modeinfo_bp, "mode_info_label", mi_pad, mi_pad, 220, info_label_height);
            send_child_rect(section_modeinfo_bp, "mode_info", mi_pad, mi_pad + info_label_height + mi_pad, mi_content_w, info_box_h);
        }
        y += h;
